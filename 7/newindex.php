...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
      try {
          ...
          $stmt = $pdo->prepare("INSERT INTO users1 (login, password, fio, phone, email, dob, gender, bio) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
          ...
          // Вставка данных в таблицу users_languages1
          ...
          echo "<div
                    ...
                    background-color:rgb(161, 64, 118);
                    border-top: 2px solid #C38DAE;
                    border-left: 2px solid #C38DAE;
                    border-right: 2px solid #C38DAE;
                    width: 50%'>
                <p>Ваш логин: " . htmlspecialchars($login) . "</p></div>";
          echo "<div style='
                    text-align: center;
                    background-color:rgb(152, 61, 111);
                    border-bottom: 2px solid #C38DAE;
                    border-left: 2px solid #C38DAE;
                    border-right: 2px solid #C38DAE;
                    width: 50%;
                    margin-bottom: 10px;'>
